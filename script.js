...
function renderGames(){
  ...
  games.forEach(g => {
    ...
    card.innerHTML = `<div class="teams"><div class="team"><div class="name">${tName(g.away)}</div><div class="score">${g.awayScore}</div></div><div class="status">${g.status}</div><div class="team"><div class="score">${g.homeScore}</div><div class="name">${tName(g.home)}</div></div></div>`;
    el.appendChild(card);
  });
}
...
function renderStandings() {
  ...
  const sorted = standings.slice().sort((a,b) => {
    const pctA=(a.w+a.l)?a.w/(a.w+a.l):0, pctB=(b.w+b.l)?b.w/(b.w+b.l):0;
    if(pctB!==pctA) return pctB-pctA; return (b.rd||0)-(a.rd||0);
  });
  sorted.forEach((s,idx)=>{
    const tr=document.createElement('tr'); const pct=(s.w+s.l)?(s.w/(s.w+s.l)).toFixed(3).slice(1):'.000';
    tr.innerHTML=`<td>${idx+1}</td><td>${tName(s.teamId)}</td><td>${s.w}</td><td>${s.l}</td><td>${s.d??0}</td><td>${pct}</td><td>${s.r??'-'}</td><td>${s.ra??'-'}</td><td>${s.rd??'-'}</td><td>${s.streak??'-'}</td>`;
    tbody.appendChild(tr);
  });
}

function renderSchedule(){
  const ul=document.getElementById('today-schedule'); if(!ul) return; ul.innerHTML='';
  games.forEach(g=>{const li=document.createElement('li');li.textContent=`${tName(g.away)} @ ${tName(g.home)} — ${g.status}`;ul.appendChild(li);});
}
